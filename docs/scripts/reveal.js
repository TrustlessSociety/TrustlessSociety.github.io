(async () => {
  ...
  class TaskRunner {
    constructor(maxThreads = 1) {
      ...
      this._hasIPFS = /Qm[1-9A-HJ-NP-Za-km-z]{44,}|b[A-Za-z2-7]{58,}|B[A-Z2-7]{58,}|z[1-9A-HJ-NP-Za-km-z]{48,}|F[0-9A-F]{50,}/
      this._hasIPFSFolder = /((Qm[1-9A-HJ-NP-Za-km-z]{44,}|b[A-Za-z2-7]{58,}|B[A-Z2-7]{58,}|z[1-9A-HJ-NP-Za-km-z]{48,}|F[0-9A-F]{50,})\/.+)/
    }
    ...
    async task(id) {
      ...
      const json = /*this._fetchIPFS(uri) ||*/ this._fetchHTTP(uri.replace('ipfs://', 'https://ipfs.io/ipfs/'))
    ...
    async _fetchHTTP(uri) {
      const response = await fetch(uri)
      return await response.json()
    }

    async _fetchIPFS(uri) {
      return
      if (!this._hasIPFS.test(uri)) {
        return
      }
      
      if (this._hasIPFSFolder.test(uri)) {
        uri = uri.match(this._hasIPFSFolder)[0]
      } else {
        uri = uri.match(this._hasIPFS)[0]
      }
